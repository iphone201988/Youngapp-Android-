...
@HiltViewModel
class FinancialBusinessVm @Inject constructor(private val apiHelper: ApiHelper) : BaseViewModel() {
    ...
    fun getBusinessPlan(url: String){
        CoroutineScope(Dispatchers.IO).launch {
            observeCommon.postValue(Resource.loading(null))
            try {
                val response = apiHelper.apiGetOnlyAuthToken(url)
                if (response.isSuccessful && response.body() != null){
                    observeCommon.postValue(Resource.success("getBusinessPlan", response.body()))
                }
                else{
